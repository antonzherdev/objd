...
public class QueueTest extends TestCase {
    public void testDeque() {
        ...
        ().assertEqualsAB<Integer>(0, ERROR: Unknown <lm>q\ImQueue#C<§^int§>\.<dI>count\uint\.cast<int>);
        q = q.enqueueItem(1);
        ().assertTrueValue(ERROR: Unknown !(<lm>q\ImQueue#C<§^int§>\.<dI>isEmpty\bool\));
        ().assertEqualsAB<Integer>(1, ERROR: Unknown <lm>q\ImQueue#C<§^int§>\.<dI>count\uint\.cast<int>);
        q = q.enqueueItem(2);
        ().assertEqualsAB<Integer>(2, ERROR: Unknown <lm>q\ImQueue#C<§^int§>\.<dI>count\uint\.cast<int>);
        q = q.enqueueItem(3);
        ().assertEqualsAB<Integer>(3, ERROR: Unknown <lm>q\ImQueue#C<§^int§>\.<dI>count\uint\.cast<int>);
