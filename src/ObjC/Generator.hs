...
tExp _ (D.Self dtp@(D.TPClass D.TPMEnum _ _)) = C.Cast (showDataType $ D.unwrapGeneric dtp) $ C.MathOp Plus (C.Arrow C.Self (C.Ref "_ordinal")) (C.IntConst 1)
	...
	in 
		if D.isTpEnum tp then 
			if check then case D.exprDataType e of
				D.TPGenericWrap _ _ -> C.Cast (showDataType tp) $ C.MathOp Plus (C.Call (C.CCall (C.Ref "nonnil") [tExp env e]) "ordinal" [] []) (C.IntConst 1)
...
maybeVal (stp, dtp) e = let 
	...
	in case (tp stp, tp dtp) of
		...
		(TPGen _, TPEnum) -> C.Cast (showDataType $ D.unwrapGeneric dtp) $ C.MathOp Plus (C.Call e "ordinal" [] []) (C.IntConst 1)
		_ -> e
